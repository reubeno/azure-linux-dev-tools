...
func PrepareForRPMBuild(env *azldev.Env) (string, error) {
	...
	for _, r := range repos {
		if !r.Repo.IsAvailableForArch(mockArch) {
			slog.Warn("Skipping rpm-repo for rpm-build (arch mismatch)",
				"repo", r.Name, "mockArch", mockArch, "repoArches", r.Repo.Arches)
...
func stageMockConfigDir(
	...
	tempDir, err := fileutils.MkdirTemp(fs, env.WorkDir(), "azldev-mock-config-")
	if err != nil {
	...
	entries, err := fileutils.ReadDir(fs, srcDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		if symErr := fileutils.SymLinkOrCopy(env, fs, src, dst, fileutils.CopyFileOptions{}); symErr != nil {
	...
	siteDefaultsContent := renderSiteDefaults(repos)
	if writeErr := fileutils.WriteFile(fs, siteDefaultsPath, []byte(siteDefaultsContent), 0o644); writeErr != nil {
...
const siteDefaultsFileName = "site-defaults.cfg"
...
func renderSiteDefaults(repos []namedRepo) string {
	...
	b.WriteString("# GENERATED BY azldev — do not edit\n")
	b.WriteString("# Sets config_opts['azl_repos'] from the active project's TOML configuration.\n")
	b.WriteString("# The chroot template is expected to iterate this list to build dnf.conf.\n\n")
	b.WriteString("config_opts['azl_repos'] = [\n")

	for _, r := range repos {
		b.WriteString("    {")
		writeStrField(&b, "name", r.Name)
		writeStrField(&b, "baseurl", r.Repo.BaseURI)
		writeStrField(&b, "metalink", r.Repo.Metalink)
		writeStrField(&b, "gpgkey", r.Repo.GPGKey)
		// Note: TOML field is `disable-gpg-check`; dnf wants `gpgcheck=1/0`.
		// Invert here so the dnf side stays normal-shaped.
		writeBoolField(&b, "gpgcheck", !r.Repo.DisableGPGCheck)
		b.WriteString("},\n")
	}
...
func writeStrField(b *strings.Builder, key, value string) {
	if value == "" {
		return
	}

	fmt.Fprintf(b, " %s: %s,", pyRepr(key), pyRepr(value))
}

func writeBoolField(b *strings.Builder, key string, value bool) {
	if value {
		fmt.Fprintf(b, " %s: True,", pyRepr(key))
	} else {
		fmt.Fprintf(b, " %s: False,", pyRepr(key))
	}
}
